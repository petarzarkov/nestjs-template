...
- **Build:** `bun run build` (`scripts/build.ts`) — a Bun-native, structure-preserving build: one `Bun.build` call passing every source file as an entrypoint with `external: ['*']`, so each file transpiles to its own output (1:1) and nothing is bundled. It does NOT bundle because Bun's bundler miscompiles legacy decorators at scale (some classes get the TC39 decorator transform instead of legacy, breaking Nest DI metadata) — per-file, `design:paramtypes` is preserved. It emits a linked `.js.map` (with embedded `sourcesContent`) next to each file, so production stack traces from `bun dist/main.js` remap to the original `.ts` lines (Bun loads the maps automatically). `Bun.build` reads the root `tsconfig.json` for jsx/decorator settings but doesn't resolve the `@/*` alias for external imports, so the script rewrites those specifiers to relative paths afterwards (`resolveAlias`, line-preserving so the maps stay valid — no `tsc-alias`). A `define` maps `process.env.NODE_ENV` to itself to defeat Bun's default build-time inlining of it (config is read from the environment at runtime, so nothing is frozen into `dist`). It also copies the Drizzle migration `.sql`/`.json` files into `dist/` so the boot-time migrator can find them. `bun run start` runs `dist/main.js`.
