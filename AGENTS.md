...
- **Linting & Formatting:** Oxlint (`bun run lint`) + oxfmt (`bun run format`) — single quotes, trailing commas, 80-char lines, custom JS plugin in `oxlint-plugins/` for TypeORM constraint naming enforcement
- **Build:** `bun run build` (`scripts/build.ts`) — a Bun-native, structure-preserving transpile (`Bun.Transpiler`), one output file per source file in `dist/`. It resolves the `@/*` alias to relative paths itself (no `tsc-alias`) and does NOT bundle (TypeORM discovers entities/migrations via runtime globs, and Bun's bundler miscompiles legacy decorators at scale). `bun run start` runs `dist/main.js`.
...
│   ├── dto/                   # Grouped config DTOs (service, db, redis, oauth, ai, aws, ws)
...
8. API docs served at `/{GLOBAL_PREFIX}/docs` (Swagger) and Scalar; `setupDocs` returns the OpenAPI document
9. `NestJsCmsModule.setup(app, document, …)` — mounts the admin CMS UI at `/cms` (after docs, before `listen`)
...
- **Config DTOs** in `config/dto/`: `ServiceVars`, `DbVars`, `RedisVars`, `OAuthVars`, `AIVars`, `AWSConfigVars`, `WsVars`
- **Config groups**: `app`, `db`, `jwt`, `redis`, `oauth`, `ai`, `aws`, `cors`, `http`, `ws`, `slack`, `email`
...
### DB Constraint Naming Convention (enforced via a custom Oxlint plugin)
...
| -------------------- | ------------- | ----------------------------------------------------------------- |
| `User`               | users         | id, email, displayName, picture, roles (`UserRole[]`), suspended  |
...
| `FileEntity`         | file          | id, name, extension, mimetype, width, height, path, userId        |
...
- **ContextLogger** — structured JSON logging with `AsyncLocalStorage`-based context (from `@arkv/nestjs-context-logger`, configured via `NestJsContextLoggerModule.forRootAsync` — see `src/config/logger.config.ts`, reused by `app.module.ts` and the queue worker `job.module.ts`)
...
- **Streams**: `warn`/`error`/`fatal` write to **stderr** (`console.error`); everything else to stdout
...
- **Dockerfile** — multi-stage build, `oven/bun:1.3.11-slim`, non-root user
