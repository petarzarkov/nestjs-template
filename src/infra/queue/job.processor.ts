...
export default async function jobProcessor(job: Job) {
  ...
  try {
    ...
    await job.log(
      `Failed to process job ${jobDispatcher.getJobId(job)}: ${error instanceof Error ? error.message : String(error)}, stack: ${error instanceof Error ? (error.stack ?? 'unknown') : 'unknown'}`,
    );
    throw error;
  }
}
