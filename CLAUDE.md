...
- **TypeScript:** Native Bun execution (no ts-node/tsx), TypeScript 6.0 for builds, target ESNext, module NodeNext
- **Typecheck:** `tsgo` (TypeScript 7 / `@typescript/native-preview`) via `bun run typecheck` — uses `tsconfig.typecheck.json` (app + e2e + scripts, excludes `*.spec.ts`)
